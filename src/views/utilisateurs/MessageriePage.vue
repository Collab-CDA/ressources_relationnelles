  ...
  methods: {
      ...
      if (this.friendId && this.userId) {
        try {
          const response = await axios.get(
            `http://192.168.243.106:3000/api/messages/${this.userId}/${this.friendId}`,
            this.getAuthHeaders()
          );
      ...
      if (this.newMessage.trim() && this.friendId && this.userId) {
        try {
          const response = await axios.post(
            `http://192.168.243.106:3000/api/messages`,
            {
    ...
    async fetchUserName() {
      if (this.userId) {
        try {
          const response = await axios.get(
            `http://192.168.243.106:3000/api/utilisateurs/${this.userId}`,
            this.getAuthHeaders()
          );
    ...
    async fetchFriendName() {
      if (this.friendId) {
        try {
          const response = await axios.get(
            `http://192.168.243.106:3000/api/utilisateurs/${this.friendId}`,
            this.getAuthHeaders()
          );
