  ...
  methods: {
      ...
      if (this.userId) {
        try {
          const response = await axios.get(
            `http://10.176.131.156:3000/api/conversations/${this.userId}`,
            this.getAuthHeaders()
          );
          ...
          this.conversations = await Promise.all(
            response.data.map(async (conversation) => {
              const friendResponse = await axios.get(
                `http://10.176.131.156:3000/api/utilisateurs/${conversation.friendId}`,
                this.getAuthHeaders()
              );
              return {
                ...conversation,
    ...
    async deleteConversation(friendId) {
      if (this.userId && friendId) {
        try {
          await axios.delete(
            `http://10.176.131.156:3000/api/conversations/${this.userId}/${friendId}`,
            this.getAuthHeaders()
          );
