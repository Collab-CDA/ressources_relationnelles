<template>
  ...
  export default {
    ...
    methods: {
      async fetchUsers() {
        try {
          const response = await axios.get('http://192.168.243.106:3000/api/users');
      ...
      async toggleStatus(user) {
        ...
        try {
          await axios.put(`http://192.168.243.106:3000/api/users/${user.id_utilisateur}`, { statut: newStatus });
      ...
      async deleteUser(userId) {
        if (confirm("Êtes-vous sûr de vouloir supprimer cet utilisateur ?")) {
          try {
            await axios.delete(`http://192.168.243.106:3000/api/users/${userId}`);
      ...
      async submitUser() {
        try {
          const response = await axios.post('http://192.168.243.106:3000/api/utilisateurs/register', {
            ...this.newUser,
            statut: 'actif',
          });
