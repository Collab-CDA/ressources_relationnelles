        ...
        const processedFiles = await Promise.all(fileNames.map(async file => {
          if (/\.(jpe?g|png|gif)$/i.test(file)) {
            try {
              ...
              return `${process.env.BASE_URL || 'http://192.168.243.106:3000'}/uploads/${file}`;
            }
...
exports.getUserResources = async (userId) => {
  try {
    ...
    throw new Error('Erreur lors de la récupération des ressources de l\'utilisateur: ' + error.message);
  }
};
